...
from src.app.transform_query_logs import QueryLog, QueryLogReader, CSVWriter, process_file
...
class TestQueryLog(unittest.TestCase):
    ...
    def test_to_csv_row(self):
        ...
        log = QueryLog(data)
        expected = ['id123', 'random_user', 'WG', 'DB', 'DB', 'SELECT * FROM table', 1024, 500]
...
class TestCSVWriter(unittest.TestCase):
    ...
    def test_write_row(self, _mock_makedirs, _mock_exists, _mock_file):
        ...
        data = ['id123', 'random_user', 'WG', 'DB', 'DB', 'SELECT * FROM table', 1024, 500]
