...
y_valid_clipped = np.clip(y_valid, quantile_transform_valid[LOWER_ALPHA], quantile_transform_valid[UPPER_ALPHA])

# Splitting training data to train and smaller validation set
X_train_final, X_valid_final, y_train_final, y_valid_final = train_test_split(X_train_poly, y_train_clipped,
...
def train_xgb_model(params, x_train_data, y_train_data, x_valid_data, y_valid_data):
    ...
    model.fit(x_train_data, y_train_data, eval_set=eval_set, early_stopping_rounds=10, verbose=False, eval_metric="mae")
...
scores = cross_val_score(best_xgb_model, X_train_poly, y_train_clipped, cv=5, scoring='neg_mean_squared_error')
